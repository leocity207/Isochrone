...
public:
    ...
    const std::vector<std::pair<std::string, std::vector<std::filesystem::path>>>& Get_Line_Files() const noexcept;
private:
    std::filesystem::path m_path_to_Station_File;
    std::vector<std::pair<std::string, std::vector<std::filesystem::path>>> m_line_data;
};
