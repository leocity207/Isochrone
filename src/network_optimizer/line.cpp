...
Line::Line(std::vector<Timetable>&& Timetable,std::string&& name) noexcept: m_schedule(std::move(Timetable)),m_name(std::move(name)),m_id(++s_count)
{
...
std::optional<Schedule_CRef> Line::Get_Schedule(const Day& matching_day,const Station& start_station,const Station& end_station) const noexcept
{
	...
	auto does_day_match_schedule = [matching_day, start_station_ref, end_station_ref](const Timetable& i) { return i.Match(matching_day) && i.Order(start_station_ref, end_station_ref); };
...
bool Line::Contain(const Station& station, const Day& matching_day) const noexcept
{
	...
	return std::any_of(temp.begin(), temp.end(), [station_ref](const Timetable& schedule) {return schedule.Contain(station_ref); });
}