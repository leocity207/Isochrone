...
std::vector<Context::Station> Reach_Algorithm::Simple::Optimize(const Context::Reach_Algorithm& algorithm_context)
{
	...
	for (size_t i = remaining_stations.size() - 1; i--;)
	{
		...
		std::for_each(std::execution::unseq,current_station_passing_lines.begin(), current_station_passing_lines.end(), [&](const Network::Scheduled_Line& line) {
			...
			std::for_each(std::execution::unseq, schedule_list.begin(), schedule_list.end(), [&](const Network::Schedule& schedule) {

				//////////////////////////////////////////////////////////////////////
				/// For all station in the lines
				/// O(S) (O(1) if paralelize but need lock inside the context station)
				std::for_each(std::execution::unseq, schedule.Get_Station_List().begin(), schedule.Get_Station_List().end(), [&](const Network::Station_CRef& station) {
					///////////////////////////////////////////////////////
					...
					if (station.get() != current_station.Get()) {
						...
						if (pose != remaining_stations.end())
							pose->Try_Set_New_Best_Time_Transport(value);
					}
				});
			});
		});
